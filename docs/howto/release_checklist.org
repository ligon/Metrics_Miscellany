...
#+end_src
- If tangles changed, re-run =./tangle.sh metrics_miscellany.org=, commit the regenerated
  =.py= files, and confirm =git status= is clean afterwards.  =make check-tangle= verifies
  the committed output still matches the org source.
