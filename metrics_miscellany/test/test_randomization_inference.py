...
def test_randomization_inference_returns_valid_pvalue():
    ...
    np.random.seed(0)
    p = float(
        mm_tests.randomization_inference(["Treatment1"], X, Y_null, VERBOSE=False)
    )
    assert 0.0 <= p <= 1.0
...
def test_randomization_inference_rejects_strong_effect():
    ...
    np.random.seed(1)
    p = float(mm_tests.randomization_inference(["Treatment1"], X, Y_alt, VERBOSE=False))
    assert p < 0.05
...
def test_randomization_inference_with_linear_restriction():
    ...
    np.random.seed(2)
    p = float(
        mm_tests.randomization_inference(
            ["Treatment1", "Treatment2"],
            X.drop("TxFemale", axis=1),
            Y_alt,
            R=R,
            VERBOSE=False,
        )
