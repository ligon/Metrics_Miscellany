...
def test_matmul(numpy_matrices, pandas_matrices, datamat_matrices):
    for A, B in [numpy_matrices, pandas_matrices, datamat_matrices]:
        ...
        if isinstance(A, dm.DataMat):
            expected = (
                dm.DataVec if getattr(B, "shape", (None, None))[1] == 1 else dm.DataMat
            )
...
def test_kron_mixed_multi_and_flat():
    ...
    A = pd.DataFrame(
        ...
        index=pd.MultiIndex.from_tuples([(0, "a"), (0, "b")], names=["i", "j"]),
        columns=pd.Index(["c0", "c1"], name="c"),
    )
    B = pd.DataFrame(
        np.arange(6).reshape(3, 2),
        index=pd.Index(range(3), name="k"),
        columns=pd.MultiIndex.from_tuples([("x", 1), ("x", 2)], names=["s", "t"]),
    )
    K = kron(A, B)
...
def test_kron_with_ndarray_operand():
    ...
    A = pd.DataFrame([[1, 2], [3, 4]], index=["r0", "r1"], columns=["c0", "c1"])
    B = np.array([[5], [6]])
    K = kron(A, B)
    assert K.shape == (4, 2)
