...
def _make_iv_dgp(N, seed):
    ...
    x["Constant"] = 1
    z["Constant"] = 1
    beta = DataMat({"Coefficients": [1.0, 0.0]}, index=["x", "Constant"])
    y = (x @ beta).squeeze() + u
    return y, x, z
...
def test_restricted_tsls_satisfying_constraint():
    ...
    np.testing.assert_allclose(V.loc["Constant", "Constant"], 0.0, atol=1e-12)
...
def test_restricted_tsls_binding_constraint():
    ...
    R = DataMat([[1.0, 0.0]], columns=["x", "Constant"], index=["c0"])
