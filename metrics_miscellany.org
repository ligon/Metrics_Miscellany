...
<<<<<<< HEAD
    ...
    True
=======
>>>>>>> parent of 9e116cc (Doctests for pinv.)
    """
    if np.isscalar(A): A = pd.DataFrame(np.array([[A]]))
...
#+begin_src python metrics_miscellany/test/test_cragg_donald.py
...
class TestFWLRegression(unittest.TestCase):
    def setUp(self):
        ...
        X2 = np.random.normal(0, 1, n) + 0.5 * X1
        X3 = np.random.normal(0, 1, n) + 0.3 * X1 + 0.2 * X2

        # Create dependent variable
        Y = 2 * X1 + 1.5 * X2 + 0.5 * X3 + np.random.normal(0, 1, n)

        # Convert to DataMat objects
        self.D = {
            'Y': dm.DataMat(Y),
            'X1': dm.DataMat(X1),
            'X2': dm.DataMat(X2),
        ...
        self.direct_coefs = np.linalg.lstsq(X, Y, rcond=None)[0][1:]  # Skip intercept
    ...
    def test_fwl_regression(self):
        ...
        X3_coef = list(B['Y'].values())[0]  # Assuming only one value in last stage
        self.assertAlmostEqual(X3_coef, self.direct_coefs[2], places=2)
    ...
    def test_reconstruct_coefficients(self):
        ...
        reconstructed = reconstruct_coefficients_from_fwl('Y', B, U)
    ...
    def test_empty_dict(self):
        ...
        reconstructed = reconstruct_coefficients_from_fwl('Y', {}, {})
