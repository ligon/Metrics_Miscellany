...
def test_diag_constructs_from_series():
    ...
    Series's index on both axes.

    NOTE: with the default =sparse=True= the off-diagonal entries
    materialize as NaN (a quirk of =pd.DataFrame.sparse.from_spmatrix=
    when the source =scipy.sparse.diags= matrix carries no fill value).
    Downstream arithmetic against this matrix would silently propagate
    NaNs.  Flagged as a follow-up; this test only checks the diagonal.
    """
    s = pd.Series([1.0, 2.0, 3.0], index=['a', 'b', 'c'])
    D = utils.diag(s)
    assert D.shape == (3, 3)
    ...
    diagonal = np.array([arr[i, i] for i in range(3)])
...
def test_sqrtm_does_not_currently_detect_indefiniteness():
    """sqrtm's PSD guard tests =np.any(s < 0)= where =s= comes from
    =np.linalg.svd= -- but singular values are non-negative by
    definition, so the guard can never fire and an indefinite input
    silently gets a wrong "square root."

    Documenting the bug here so it's not silently re-introduced if
    someone "fixes" the test.  When the guard is repaired (by
    inspecting eigenvalues via =eigh= rather than singular values),
    this test should flip to =pytest.raises(ValueError, ...)= and
    the docstring above should be removed.
    """
    bad = pd.DataFrame(np.diag([1.0, -1.0]), index=['a', 'b'],
                       columns=['a', 'b'])
    # No exception today:
    S = utils.sqrtm(bad, hermitian=True)
    # And the result fails the defining property S @ S.T == bad:
    recon = S.values @ S.values.T
    assert not np.allclose(recon, bad.values), (
        "If this assertion ever flips to True, the bug is gone and "
        "the test should be rewritten to check for the ValueError.")
