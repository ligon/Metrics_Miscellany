...
def svd_missing(A,max_rank=None,min_obs=None,heteroskedastic=False,verbose=False):
    ...
    if heteroskedastic: # Interpret tuple
        try:
            ...
            pass
        Pbar = P.mean()
        u,s = heteropca(P-Pbar,r=r,max_its=max_its,tol=tol,verbose=verbose)
    ...
    v = pd.DataFrame(v,columns=A.columns)
...
\[
     Y = BX + \Lambda F + U
\]
where $Y$ and $U$ are $k\times 1$, $X$ is $\ell\times 1$, $B$ is $k\times\ell$, $F$ is $r\times 1$, and \Lambda is $k\times r$.  Only $(Y,X)$ are observed; $F$ is a collection of latent "factors."  The identifying assumptions are that $U$ is orthogonal to $(X,F)$ and $\E FF^\T = I$.  [cite/t:@hansen20:econometrics] describes an iterative approach to estimation, which we implement below.
* GMM
