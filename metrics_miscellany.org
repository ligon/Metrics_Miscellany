...
def kron(A,B,sparse=False):
    ...
    a = np.kron(A,B)
    columns = pd.MultiIndex.from_product(A.columns.levels+B.columns.levels)

    index = pd.MultiIndex.from_product(A.index.levels+B.index.levels)
...
class DataMat(pd.DataFrame):
    ...
    def concat(self,other,axis=0,levelnames=False,suffixer='_',**kwargs):
        ...
        if isinstance(other,tuple):
            ...
            allobjs = [self] + other.values()
            allnames = [self.name] + other.keys()
        ...
        if levelnames:
            return utils.concat(d,axis=axis,**kwargs)
        else:
...
def reconcile_indices(idxs,fillvalue=''):
    """
    ...
    for x in idxs:
        ...
        for i,level in enumerate(x.levels):
            if len(level)==1: # Vestigial level
                try:
                    if len(x.levels)>1:
                        dropname = x.names[i]
