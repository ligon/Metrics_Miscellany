...
normally distributed), and is due to Maunchly (1940)[fn:: See
...
def maunchly(C,N):
    ...
    d = 1 - (2*(m-1)**2 + (m-1) + 2)/(6*(m-1)*(N-1))

    x2 = -(N-1)*d*V  # Chi-squared statistic

    df = m*(m-1)/2 - 1

    return 1 - chi2.cdf(x2,df)
