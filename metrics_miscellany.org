...
import pandas as pd
from metrics_miscellany.utils import matrix_product
...
#+begin_src python :tangle metrics_miscellany/estimators.py
from metrics_miscellany import DataMat
...
def linear_gmm(X,y,Z,W=None):
    ...
    Omega = Z.T@diag(e**2)@Z/n #pd.DataFrame(Z.T.values@np.diag(e**2)@Z.values/n,columns=Z.columns,index=Z.columns)
    #Omega = pd.DataFrame(e.var()*Z.T.values@Z.values/n,columns=Z.columns,index=Z.columns)
