...
def drop_vestigial_levels(idx,axis=0,both=False,multiindex=True):
    ...
    try:
        ...
        if multiindex: # Return a multiindex, not an Index
...
def restricted_tsls(y,X,R=None,r=None,Z=None,cov='HC3'):
    ...
    QRT = dm.concat({'beta':Q,'lm':R.T},axis=1,toplevelname=True)
    R0 = dm.concat({'beta':R,
                    'lm':dm.DataMat(index=R.index,columns=R.index).fillna(0)},axis=1,toplevelname=True)
    lhs = dm.concat({'beta':QRT,'lm':R0})
    rhs = dm.concat({'beta':Qxz@(Qzzinv)@Z.T@y,'lm':r})
    ...
    Vb = Dinv@(Qxz@Qzzinv@Omega@Qzzinv@Qxz.T)@Dinv/N

    if cov=='HC1':
        Vb = Vb*(N/(N-k))

    return b,lm,Omega,Vb
#+end_src
#+begin_src python :tangle metrics_miscellany/estimators.py
