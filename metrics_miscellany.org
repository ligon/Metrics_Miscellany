...
def gj(b,data):
    ...
    Returns an Nxl matrix (observations x moment conditions).
...
def continuously_updated_gmm(data,b_init=None):
    ...
    # First step uses identity weighting matrix
    W = lambda b: np.linalg.inv(Omegahat(b,data))

    bhat = minimize(lambda b: J(b,W(b),data),b_init=b_init)

    return bhat, J(bhat,W(bhat),data)
