...
def matrix_product(X,Y):
    ...
    return np.matmul(pd.DataFrame(X).fillna(0),pd.DataFrame(Y).fillna(0))
...
def kron(A,B,sparse=False):
    ...
    a = np.kron(A,B)
    return pd.DataFrame(a,columns=pd.MultiIndex.from_product([A.columns, B.columns]),
                        index=pd.MultiIndex.from_product([A.index, B.index]))
...
class DataMat(pd.DataFrame):
    ...
    def kron(self,other,sparse=False):
        return utils.kron(self,other,sparse=sparse)
