...
- The tangled =.py= files are committed, so that =pip install git+= yields an importable
  package.  Commit the regenerated files together with the Org change; =make check-tangle=
